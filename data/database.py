...
def fetch_risk_events(
    ...
    try:
        conn = get_connection()
        cutoff = _cutoff(days)
        where_clauses = ["c.last_seen >= ?"]
        params: list = [cutoff]

        if risk_types:
            ph = ",".join("?" * len(risk_types))
...
def fetch_sentiment_trend(days: int = 14) -> pd.DataFrame:
    try:
        ...
        df = pd.read_sql_query(
            """
            SELECT date(e.published_at) AS date, ac.asset_class,
...
def fetch_composite_trend(days: int = 14) -> pd.DataFrame:
    try:
        ...
        df = pd.read_sql_query(
            """
            SELECT date(c.last_seen) AS date, rt.risk_type,
...
def has_data() -> bool:
    """Return True if the database has at least one scored event."""
    try:
        conn = get_connection()
        cur = conn.execute("SELECT COUNT(*) FROM event_clusters")
        n = cur.fetchone()[0]
        conn.close()
        return n > 0
    except Exception:
